<script lang="ts">
  ...
  let { posts = [], postsPerPage = 6 }: { posts?: Post[]; postsPerPage?: number } = $props();
  
  let currentPage = $state(1);
  
  // Calculate pagination values using $derived
  const totalPages = $derived(Math.ceil(posts.length / postsPerPage));
  const startIndex = $derived((currentPage - 1) * postsPerPage);
  const endIndex = $derived(startIndex + postsPerPage);
  const showPagination = $derived(posts.length > postsPerPage);
  
  // Create placeholder posts to maintain consistent 2-row layout (6 posts)
  const displayPosts = $derived((() => {
    const postsForPage = posts.slice(startIndex, endIndex);
    const placeholders = Array(Math.max(0, postsPerPage - postsForPage.length)).fill(null);
    return [...postsForPage, ...placeholders];
  })());
...
<section id="posts-section" class="bg-gray-50">
